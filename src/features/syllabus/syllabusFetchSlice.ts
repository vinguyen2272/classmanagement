...
export const syllabusFetchSlice = createAppSlice({
  ...
  reducers: {
    ...
    deleteSyllabus: (state, action) => {
      state.syllabuses = state.syllabuses.filter(
        item => item.syllabusId !== action.payload
      )
    },
  },
  extraReducers(builder) {
    ...
    builder.addCase(getSyllabusById.fulfilled, (state, action) => {
      const syllabusIndex = state.syllabuses.findIndex(
        s => s.syllabusId === action.payload.syllabusId
      )
      if (syllabusIndex !== -1) {
