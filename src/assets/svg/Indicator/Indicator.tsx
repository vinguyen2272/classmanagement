import type { IconProps } from '../../../context/Interface/props'

export const WarringIcon: React.FC<IconProps> = ({
  color = 'green',
  fill = 'none',
  width = 20,
  height = 20,
}) => (
  <svg
    width={width}
    height={height}
    viewBox="0 0 20 20"
    fill={color}
    xmlns="http://www.w3.org/2000/svg"
  >
    <path
      d="M10 20C15.5234 20 20 15.5234 20 10C20 4.47656 15.5234 0 10 0C4.47656 0 0 4.47656 0 10C0 15.5234 4.47656 20 10 20ZM10.9375 14.0625C10.9375 14.5781 10.5156 15 10 15C9.48438 15 9.0625 14.5801 9.0625 14.0625L9.0625 9.0625C9.0625 8.54492 9.48242 8.125 10 8.125C10.5176 8.125 10.9375 8.54297 10.9375 9.0625L10.9375 14.0625ZM10 4.375C10.6781 4.375 11.2281 4.925 11.2281 5.60312C11.2281 6.28125 10.6785 6.83125 10 6.83125C9.32148 6.83125 8.77188 6.28125 8.77188 5.60312C8.77344 4.92578 9.32031 4.375 10 4.375Z"
      fill={color}
      stroke={fill}
    />
  </svg>
)

export const ReportProblemIcon: React.FC<IconProps> = ({
  color = '#2D3748',
  fill = 'none',
  width = 24,
  height = 24,
}) => (
  <svg
    width={width}
    height={height}
    viewBox="0 0 24 24"
    fill={color}
    xmlns="http://www.w3.org/2000/svg"
  >
    <g clipPath="url(#clip0_184_16128)">
      <path
        d="M12 5.99L19.53 19H4.47L12 5.99ZM12 2L1 21H23L12 2ZM13 16H11V18H13V16ZM13 10H11V14H13V10Z"
        fill={color}
        stroke={fill}
      />
    </g>
    <defs>
      <clipPath id="clip0_184_16128">
        <rect width={width} height={height} fill={color} />
      </clipPath>
    </defs>
  </svg>
)

export const InfoIcon: React.FC<IconProps> = ({
  color = '#2D3748',
  fill = 'none',
  width = 24,
  height = 24,
}) => (
  <svg
    width={width}
    height={height}
    viewBox="0 0 24 24"
    fill={color}
    xmlns="http://www.w3.org/2000/svg"
  >
    <g clipPath="url(#clip0_184_16132)">
      <path
        d="M11 7H13V9H11V7ZM11 11H13V17H11V11ZM12 2C6.48 2 2 6.48 2 12C2 17.52 6.48 22 12 22C17.52 22 22 17.52 22 12C22 6.48 17.52 2 12 2ZM12 20C7.59 20 4 16.41 4 12C4 7.59 7.59 4 12 4C16.41 4 20 7.59 20 12C20 16.41 16.41 20 12 20Z"
        fill={color}
        stroke={fill}
      />
    </g>
    <defs>
      <clipPath id="clip0_184_16132">
        <rect width={width} height={height} fill={color} />
      </clipPath>
    </defs>
  </svg>
)

export const VerifiedUserIcon: React.FC<IconProps> = ({
  color = '#2D3748',
  fill = 'none',
  width = 24,
  height = 24,
}) => (
  <svg
    width={width}
    height={height}
    viewBox="0 0 24 24"
    fill={color}
    xmlns="http://www.w3.org/2000/svg"
  >
    <g clipPath="url(#clip0_184_16136)">
      <path
        d="M12 1L3 5V11C3 16.55 6.84 21.74 12 23C17.16 21.74 21 16.55 21 11V5L12 1ZM19 11C19 15.52 16.02 19.69 12 20.93C7.98 19.69 5 15.52 5 11V6.3L12 3.19L19 6.3V11ZM7.41 11.59L6 13L10 17L18 9L16.59 7.58L10 14.17L7.41 11.59Z"
        fill={color}
        stroke={fill}
      />
    </g>
    <defs>
      <clipPath id="clip0_184_16136">
        <rect width={width} height={height} fill={color} />
      </clipPath>
    </defs>
  </svg>
)

export const FilterCenterFocusIcon: React.FC<IconProps> = ({
  color = '#2D3748',
  fill = 'none',
  width = 24,
  height = 24,
}) => (
  <svg
    width={width}
    height={height}
    viewBox="0 0 24 24"
    fill={color}
    xmlns="http://www.w3.org/2000/svg"
  >
    <g clipPath="url(#clip0_184_16140)">
      <path
        d="M5 15H3V19C3 20.1 3.9 21 5 21H9V19H5V15ZM5 5H9V3H5C3.9 3 3 3.9 3 5V9H5V5ZM19 3H15V5H19V9H21V5C21 3.9 20.1 3 19 3ZM19 19H15V21H19C20.1 21 21 20.1 21 19V15H19V19ZM12 9C10.34 9 9 10.34 9 12C9 13.66 10.34 15 12 15C13.66 15 15 13.66 15 12C15 10.34 13.66 9 12 9Z"
        fill={color}
        stroke={fill}
      />
    </g>
    <defs>
      <clipPath id="clip0_184_16140">
        <rect width={width} height={height} fill={color} />
      </clipPath>
    </defs>
  </svg>
)

export const CheckBoxIcon: React.FC<IconProps> = ({
  color = '#2D3748',
  fill = 'none',
  width = 24,
  height = 24,
}) => (
  <svg
    width={width}
    height={height}
    viewBox="0 0 24 24"
    fill={color}
    xmlns="http://www.w3.org/2000/svg"
  >
    <g clipPath="url(#clip0_184_16144)">
      <path
        d="M19 3H5C3.9 3 3 3.9 3 5V19C3 20.1 3.9 21 5 21H19C20.1 21 21 20.1 21 19V5C21 3.9 20.1 3 19 3ZM19 19H5V5H19V19ZM17.99 9L16.58 7.58L9.99 14.17L7.41 11.6L5.99 13.01L9.99 17L17.99 9Z"
        fill={color}
        stroke={fill}
      />
    </g>
    <defs>
      <clipPath id="clip0_184_16144">
        <rect width={width} height={height} fill={color} />
      </clipPath>
    </defs>
  </svg>
)

export const CheckBoxOutlineBlankIcon: React.FC<IconProps> = ({
  color = '#2D3748',
  fill = 'none',
  width = 24,
  height = 24,
}) => (
  <svg
    width={width}
    height={height}
    viewBox="0 0 24 24"
    fill={color}
    xmlns="http://www.w3.org/2000/svg"
  >
    <g clipPath="url(#clip0_184_16148)">
      <path
        d="M19 5V19H5V5H19ZM19 3H5C3.9 3 3 3.9 3 5V19C3 20.1 3.9 21 5 21H19C20.1 21 21 20.1 21 19V5C21 3.9 20.1 3 19 3Z"
        fill={color}
        stroke={fill}
      />
    </g>
    <defs>
      <clipPath id="clip0_184_16148">
        <rect width={width} height={height} fill={color} />
      </clipPath>
    </defs>
  </svg>
)

export const RadioButtonCheckedIcon: React.FC<IconProps> = ({
  color = '#2D3748',
  fill = 'none',
  width = 24,
  height = 24,
}) => (
  <svg
    width={width}
    height={height}
    viewBox="0 0 24 24"
    fill={color}
    xmlns="http://www.w3.org/2000/svg"
  >
    <g clipPath="url(#clip0_184_16152)">
      <path
        d="M12 2C6.48 2 2 6.48 2 12C2 17.52 6.48 22 12 22C17.52 22 22 17.52 22 12C22 6.48 17.52 2 12 2ZM12 20C7.58 20 4 16.42 4 12C4 7.58 7.58 4 12 4C16.42 4 20 7.58 20 12C20 16.42 16.42 20 12 20Z"
        fill={color}
        stroke={fill}
      />
      <path
        d="M12 17C14.7614 17 17 14.7614 17 12C17 9.23858 14.7614 7 12 7C9.23858 7 7 9.23858 7 12C7 14.7614 9.23858 17 12 17Z"
        fill={color}
        stroke={fill}
      />
    </g>
    <defs>
      <clipPath id="clip0_184_16152">
        <rect width={width} height={height} fill={color} />
      </clipPath>
    </defs>
  </svg>
)

export const RadioButtonUnCheckedIcon: React.FC<IconProps> = ({
  color = '#2D3748',
  fill = 'none',
  width = 24,
  height = 24,
}) => (
  <svg
    width={width}
    height={height}
    viewBox="0 0 24 24"
    fill={color}
    xmlns="http://www.w3.org/2000/svg"
  >
    <g clipPath="url(#clip0_184_16157)">
      <path
        d="M12 2C6.48 2 2 6.48 2 12C2 17.52 6.48 22 12 22C17.52 22 22 17.52 22 12C22 6.48 17.52 2 12 2ZM12 20C7.58 20 4 16.42 4 12C4 7.58 7.58 4 12 4C16.42 4 20 7.58 20 12C20 16.42 16.42 20 12 20Z"
        fill={color}
        stroke={fill}
      />
    </g>
    <defs>
      <clipPath id="clip0_184_16157">
        <rect width={width} height={height} fill={color} />
      </clipPath>
    </defs>
  </svg>
)

export const GradeIcon: React.FC<IconProps> = ({
  color = '#2D3748',
  fill = 'none',
  width = 24,
  height = 24,
}) => (
  <svg
    width={width}
    height={height}
    viewBox="0 0 24 24"
    fill={color}
    xmlns="http://www.w3.org/2000/svg"
  >
    <g clipPath="url(#clip0_184_16161)">
      <path
        d="M12 7.13L12.97 9.42L13.44 10.53L14.64 10.63L17.11 10.84L15.23 12.47L14.32 13.26L14.59 14.44L15.15 16.85L13.03 15.57L12 14.93L10.97 15.55L8.85 16.83L9.41 14.42L9.68 13.24L8.77 12.45L6.89 10.82L9.36 10.61L10.56 10.51L11.03 9.4L12 7.13ZM12 2L9.19 8.63L2 9.24L7.46 13.97L5.82 21L12 17.27L18.18 21L16.54 13.97L22 9.24L14.81 8.63L12 2Z"
        fill={color}
        stroke={fill}
      />
    </g>
    <defs>
      <clipPath id="clip0_184_16161">
        <rect width={width} height={height} fill={color} />
      </clipPath>
    </defs>
  </svg>
)

export const SupplierIcon: React.FC<IconProps> = ({
  color = '#2D3748',
  fill = 'none',
  width = 24,
  height = 24,
}) => (
  <svg
    width={width}
    height={height}
    viewBox="0 0 24 24"
    fill={color}
    xmlns="http://www.w3.org/2000/svg"
  >
    <g clipPath="url(#clip0_184_16164)">
      <path
        d="M11.99 2C6.47 2 2 6.48 2 12C2 17.52 6.47 22 11.99 22C17.52 22 22 17.52 22 12C22 6.48 17.52 2 11.99 2ZM19.47 9.16L14.46 8.73L12.46 4.02C15.67 4.21 18.37 6.29 19.47 9.16ZM14.4 15.42L12 13.98L9.61 15.42L10.24 12.7L8.13 10.87L10.91 10.63L12 8.06L13.09 10.62L15.87 10.86L13.76 12.69L14.4 15.42ZM11.54 4.02L9.54 8.74L4.52 9.17C5.62 6.29 8.32 4.2 11.54 4.02ZM4 12C4 11.36 4.08 10.74 4.23 10.14L8.02 13.42L6.91 18.17C5.13 16.7 4 14.48 4 12ZM7.84 18.82L12 16.31L16.16 18.81C14.94 19.56 13.52 20 11.99 20C10.47 20 9.05 19.56 7.84 18.82ZM17.09 18.17L15.98 13.42L19.77 10.14C19.91 10.73 20 11.36 20 12C20 14.48 18.86 16.7 17.09 18.17Z"
        fill={color}
        stroke={fill}
      />
    </g>
    <defs>
      <clipPath id="clip0_184_16164">
        <rect width={width} height={height} fill={color} />
      </clipPath>
    </defs>
  </svg>
)
